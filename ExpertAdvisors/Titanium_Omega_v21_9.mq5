...
input ENUM_TIMEFRAMES HigherTF    = PERIOD_H4; // MTF Onayı
...
input bool     InpManageManual    = false;    // Manuel İşlemleri de Yönet
...
public:
   ...
   bool IsSafeToTrade()
   {
      ...
      if(!g_StateReason.get_Name().Compare("AKTİF") || spreadPips > InpMaxSpreadPips || marginLevel < InpMinMarginLevel)
         Print(log);
...
public:
   ...
   int GetDirection(ENUM_MARKET_REGIME regime)
   {
      ...
      if(regime == REGIME_TRENDING)
      {
         ...
         {
            if(isDip && bufClose[0] < bufMA[0]) 
            {
               g_StateReason = "TREND FİLTRESİ (FİYAT < MA)";
               return 0;
            }
            if(isTop && bufClose[0] > bufMA[0]) 
            {
               g_StateReason = "TREND FİLTRESİ (FİYAT > MA)";
               return 0;
            }
      ...
      if(regime == REGIME_TRENDING)
      {
         ...
         double price = bufClose[0];
         sigLog += "   • Trend Filtresi: Fiyat(" + DoubleToString(price, 5) + ") " + (price > maVal ? ">" : "<") + " MA(" + DoubleToString(maVal, 5) + ") -> " + (price > maVal ? "YUKARI" : "AŞAĞI");
      }
...
public:
   ...
   int CalculateSafeOrderCount(int direction)
   {
      ...
      double marginReq = 0;
      if(!OrderCalcMargin(type, _Symbol, InpFixedLot, price, marginReq)) 
         return 0;
      ...
      double lossPerTrade = InpFixedLot * InpSL_Pips * pipValue;
   ...
   void PlaceGrid(int direction)
   {
      ...
      for(int i = 0; i < count; i++)
      {
         ...
         if(direction == 1) // BUY
         {
            ...
            if(!m_trade.BuyStop(InpFixedLot, NormalizeDouble(entry, digits), _Symbol, 
         ...
         else // SELL
         {
            ...
            if(!m_trade.SellStop(InpFixedLot, NormalizeDouble(entry, digits), _Symbol, 
               NormalizeDouble(sl, digits), NormalizeDouble(tp, digits), 
