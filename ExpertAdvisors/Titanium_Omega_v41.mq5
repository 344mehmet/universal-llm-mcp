//+------------------------------------------------------------------+
//|                                     Titanium_Omega_v41.mq5       |
//|                     © 2025, Systemic Trading Engineering         |
//|  Versiyon: 41.0 (MA MASTER - SMA+EMA UNLIMITED PROFIT)          |
//+------------------------------------------------------------------+
...
#include "Titanium_Omega_v41_MA_Engine.mqh"
...
enum ENUM_STRATEGY_MODE {
   ...
   STRATEGY_MA_MASTER         // v41: SMA+EMA Trend & Pullback System
};
...
input string   InpComment         = "Titanium Omega v41"; // İşlem Yorumu
input bool     InpShowDashboard   = true;      // Bilgi Paneli Göster
input ENUM_STRATEGY_MODE InpStrategyMode = STRATEGY_MA_MASTER; // v41: MA MASTER VARSAYILAN
...
input int      InpSL_Pips         = 15;       // v40: Stop Loss (Pips) - Scalping
...
//--- 8. MA MASTER SETTINGS (v41)
input group "=== 8. MA MASTER SETTINGS (v41) ==="
...
int OnInit()
{
   ...
   // v41 - MA Master Engine
   if(!MAMaster.Init()) return INIT_FAILED;
   
   Print("✅ v41: Tüm modüller başlatıldı (MA MASTER UNLIMITED).");
   
   // v41 - Başlangıç Mesajı
   Print("=================================================");
   Print("TITANIUM OMEGA v41.0 - MA MASTER (UNLIMITED)");
   Print("=================================================");
   ...
   Print("♻️ Pyramiding: ", InpPyramiding ? "AKTIF (Trende Ekleme)" : "PASIF");
   Print("✅ Mod: MA MASTER (SMA+EMA)");
   Print("=================================================");
...
void OnTick()
{
   ...
   if(signal == 0)
   {
      ...
      {
         // v41: MA Master Sinyali (SMA + EMA)
         signal = MAMaster.GetMasterSignal();
      }
      else
      {
         // Eski Stratejiler
   ...
   if(InpShowDashboard || InpForceShowDashboard)
   {
      ...
      else if(InpStrategyMode == STRATEGY_MA_MASTER) stratName = "MA MASTER (UNLIMITED)";
      ...
      string dash = "+================================================+\n";
      dash += "|   TITANIUM OMEGA v41.0 MA MASTER UNLIMITED     |\n";
      dash += "+================================================+\n";
