...
//--- 7. RULE ENFORCER (GÜVENLİK v24)
input group "=== 7. TANK SECURITY (v24) ==="
...
private:
   ...
   int               m_hBands;
   int               m_hADX;
   int               m_hMA_Curr; // Artık kullanılmıyor (HMA'ya geçildi)
   int               m_hMA_High; // Artık kullanılmıyor
...
public:
   ...
   // HMA Hesaplama Fonksiyonu (Custom)
   double CalculateHMA(int shift)
   {
      int sqrtPeriod = (int)MathSqrt(m_hmaPeriod);
      int lookback = sqrtPeriod + 1; // WMA hesaplamak için yeterli veri
      
      double wmaHalf[], wmaFull[];
      ArraySetAsSeries(wmaHalf, true);
      ArraySetAsSeries(wmaFull, true);
      
      if(CopyBuffer(m_hWMA_Half, 0, shift, lookback, wmaHalf) < lookback) return 0;
      if(CopyBuffer(m_hWMA_Full, 0, shift, lookback, wmaFull) < lookback) return 0;
      
      // Raw HMA Serisini Oluştur: 2 * WMA(n/2) - WMA(n)
      double rawHMA[];
