//+------------------------------------------------------------------+
//|                                     Titanium_Omega_v35.mq5       |
//|                     © 2025, Systemic Trading Engineering         |
//|  Versiyon: 35.0 (ADVANCED TRADING - LIMIT ORDERS + ADAPTIVE)    |
//+------------------------------------------------------------------+
...
input string   InpComment         = "Titanium Omega v35"; // İşlem Yorumu
...
//--- 10. TEST & DEBUG
input group "=== 10. TEST & DEBUG (v35) ==="
input bool     InpTestMode             = true;     // v35: Test Modu (Hızlı İşlem)
input bool     InpRelaxChecks          = true;     // v35: Tester'da Kontrolleri Gevşet
...
input ENUM_ORDER_TYPE_MODE InpOrderMode = ORDER_MODE_MARKET; // Emir Tipi
...
public:
   ...
   {
      Print("🔵 v35: PlaceGrid başlatıldı. Yön: ", (direction == 1 ? "BUY" : "SELL"), " | Emir Tipi: ", EnumToString(InpOrderMode));
      ...
      {
         Print("🔵 v35: Market emir modu seçili.");
         double sl = 0, tp = 0;
         ...
         if(direction == 1) // BUY
         {
            ...
            Print("🚀 v35: Market BUY açılıyor... Lot:", DoubleToString(lotToUse, 2), 
            ...
            {
               g_tradesTodayCount++;
               Print("✅ v35: Market BUY başarılı! Ticket:", m_trade.ResultOrder());
            }
            else
            {
         ...
         else // SELL
         {
            ...
            Print("🚀 v35: Market SELL açılıyor... Lot:", DoubleToString(lotToUse, 2), 
                  " SL:", DoubleToString(sl, digits), " TP:", DoubleToString(tp, digits));
            ...
            {
               g_tradesTodayCount++;
               Print("✅ v35: Market SELL başarılı! Ticket:", m_trade.ResultOrder());
            }
            else
            {
...
int OnInit()
{
   ...
   Print("✅ v35: Tüm modüller başlatıldı.");
   
   // v35 - Başlangıç Mesajı
   Print("=");
   Print("TITANIUM OMEGA v35.0 ADVANCED TRADING BAŞLATILDI");
   Print("Bakıye: ", AccountInfoDouble(ACCOUNT_BALANCE), " ", AccountInfoString(ACCOUNT_CURRENCY));
   Print("Emir Tipi: ", EnumToString(InpOrderMode));
   Print("Limit Offset: ", InpLimitOffset, " pips (ATR: ", InpUseDynamicLimitOffset ? "AKTIF" : "PASIF", ")");
   Print("=");
...
void OnTick()
{
   ...
   Print("🔍 v35 DEBUG: safeToOpen=", safeToOpen, " | Reason=", g_StateReason);
   ...
   Print("🔍 v35 DEBUG: signal=", signal, " | InpUseConsensus=", InpUseConsensus);
   ...
   {
      Print("🔍 v35 DEBUG: Trade koşulları kontrol ediliyor... signal=", signal);
      ...
      if(safeToOpen)
      {
         Print("🔍 v35 DEBUG: RR kontrolü geçti, pozisyon sayısı kontrol ediliyor...");
   ...
   if(InpShowDashboard || InpForceShowDashboard)
   {
      ...
      string dash = "+================================================+\n";
      dash += "|   TITANIUM OMEGA v35.0 ADVANCED TRADING       |\n";
      dash += "+================================================+\n";
