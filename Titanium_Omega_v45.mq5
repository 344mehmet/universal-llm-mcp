//+------------------------------------------------------------------+
//|                                     Titanium_Omega_v45.mq5       |
//|                     © 2025, Systemic Trading Engineering         |
//|  Versiyon: 45.0 (DEBUG MODE - FULL VERBOSE LOGGING)             |
//+------------------------------------------------------------------+
...
public:
   ...
   {
      Print("DEBUG: GetDirection Called. Mode: ", EnumToString(InpStrategyMode));
      
      // v42: MA Master (Native) - ÖNCELİKLİ KONTROL
      if(InpStrategyMode == STRATEGY_MA_MASTER)
      {
         int sig = GetMAMasterSignal();
         Print("DEBUG: MA Master Result: ", sig);
         return sig;
      }
   ...
   int GetMAMasterSignal()
   {
      ...
      if(c1 < 1 || c2 < 2 || c3 < 2)
      {
         Print("DEBUG: [MA MASTER] Yetersiz Veri! SMA:", c1, " Fast:", c2, " Slow:", c3);
         return 0;
      }
      ...
      // DEBUG LOGS v45
      Print("DEBUG: [MA MASTER] Price: ", price, " SMA200: ", smaTrend[0]);
      Print("DEBUG: [MA MASTER] FastEMA: ", fast[0], " (Prev: ", fast[1], ")");
      Print("DEBUG: [MA MASTER] SlowEMA: ", slow[0], " (Prev: ", slow[1], ")");
      ...
      Print("DEBUG: [MA MASTER] Trend Yönü: ", trend);
...
public:
   ...
   bool CheckRiskReward(double entry, double sl, double tp, int direction)
   {
      ...
      Print("DEBUG: R:R Check -> Risk: ", DoubleToString(risk, 5), " Reward: ", DoubleToString(reward, 5), " Ratio: ", DoubleToString(rr, 2), " Min: ", DoubleToString(InpMinRiskReward, 2));
      
      if(rr < InpMinRiskReward)
      {
         g_StateReason = "DÜŞÜK R:R ORANI (" + DoubleToString(rr, 2) + " < " + DoubleToString(InpMinRiskReward, 2) + ")";
         // v46: Debug modda R:R kontrolünü yoksay (Kullanıcı işlem görmek istiyor)
         Print("⚠️ v46: R:R Düşük ama Debug Modu için işlem açılıyor!");
         return true; // DEBUG İÇİN DEĞİŞTİRİLDİ
      }
      return true;
   }
};
...
public:
   ...
   void PlaceGrid(int direction)
   {
      ...
      if(!InpRelaxChecks)
      {
         ...
         Print("DEBUG: [GRID] Safe Order Count: ", count);
         if(count <= 0) 
         {
...
void OnTick()
{
   Print("DEBUG: === OnTick START [", TimeToString(TimeCurrent()), "] ===");
   ...
   if(safeToOpen && Fund.ShouldPauseTrading()) 
   {
      Print("DEBUG: İşlem Durduruldu: Fon Yönetimi Hedefi");
      safeToOpen = false;
   }
   ...
   if(InpMaxTradesPerDay > 0 && g_tradesTodayCount >= InpMaxTradesPerDay)
   {
      ...
      Print("DEBUG: İşlem Durduruldu: Günlük Limit");
   }
   ...
   // Haber Filtresi
   if(safeToOpen && News.IsNewsTime()) 
   {
      Print("DEBUG: İşlem Durduruldu: Haber Filtresi");
      safeToOpen = false;
   }
   
   if(!safeToOpen)
   {
      Print("DEBUG: safeToOpen = FALSE. Neden: ", g_StateReason);
   }
   ...
   if(signal != 0) Print("DEBUG: Reverse Sinyal Tespit Edildi: ", signal);
   ...
   if(signal == 0)
   {
      ...
      if(InpStrategyMode == STRATEGY_MA_MASTER)
      {
         int sig = Signal.GetMAMasterSignal();
         Print("DEBUG: MA Master Result: ", sig);
         signal = sig;
      }
      else
   ...
   Print("DEBUG: Final Sinyal: ", signal);
   ...
   if(safeToOpen && signal != 0)
   {
      ...
      if(safeToOpen)
      {
         ...
         {
            safeToOpen = false;
            Print("DEBUG: Risk/Reward Engeli");
         }
      }
      
      if(safeToOpen)
      {
         Print("DEBUG: İşlem Açılıyor...");
         ...
         {
             Print("DEBUG: Max Emir Limiti: ", currentOpenOrders, "/", maxOrders);
         }
      }
   }
   ...
   if(InpShowDashboard || InpForceShowDashboard)
   {
      ...
      string dash = "+================================================+\n";
      dash += "|   TITANIUM OMEGA v45.0 DEBUG MODE              |\n";
      dash += "+================================================+\n";
