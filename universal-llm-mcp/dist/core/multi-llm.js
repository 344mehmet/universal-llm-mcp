 ...
 */
export async function queryAllLLMs(question, systemPrompt) {
    const router = getRouter();
    const backends = ['lmstudio', 'ollama'];
    ...
    const promises = backends.map(async (backend) => {
        ...
        try {
            const response = await router.completeWithBackend(backend, question, systemPrompt || 'Sen yardimci bir asistansin. Detayli ve kapsamli yanit ver. Sinir yok.');
            return {
                backend,
...
//# sourceMappingURL=multi-llm.js.map