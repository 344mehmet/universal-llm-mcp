...
export class RAGService {
     ...
     */
    async addDocument(text, source, category) {
        try {
            ...
            for (const chunk of chunks) {
                if (chunk.text.trim().length < 10)
                    continue; // Çok kısa chunk'ları atla
                try {
    ...
    async query(question, options) {
        ...
        try {
            ...
            const searchResults = this.vectorStore.search(queryEmbedding, topK, options?.category ? { category: options.category } : undefined);
     ...
     */
    async askLLMWithContext(question, context) {
...
- Kaynaklara atıfta bulun`;
        const response = await router.complete('default', prompt, 'Sen bir bilgi asistanısın. Verilen bağlamdan doğru ve öz bilgiler çıkar, Türkçe cevap ver.');
