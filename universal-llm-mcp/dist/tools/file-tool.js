...
`;
    const response = await router.complete('file', prompt, 'Sen uzman bir dosya ve kod analiz uzmanısın. Detaylı ve yapıcı analizler yap.');
...
Türkçe olarak yaz.
`;
    const response = await router.complete('file', prompt, 'Sen teknik dokümantasyon uzmanısın. Anlaşılır ve kapsamlı dokümantasyon yaz.');
    return response.content;
}
/**
 * Dosyaları karşılaştır
 */
export async function compareFiles(args) {
    const router = getRouter();
...
`;
    const response = await router.complete('file', prompt, 'Sen dosya karşılaştırma uzmanısın. Değişiklikleri net ve anlaşılır şekilde raporla.');
    return response.content;
}
/**
...
export function registerFileTools(server) {
    server.tool('dosya_analiz', 'Bir dosyayı analiz et (dosya yolu ile)', analyzeFileSchema.shape, async (args) => {
        const sonuc = await analyzeFile(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('icerik_analiz', 'Verilen içeriği analiz et', analyzeContentSchema.shape, async (args) => {
        const sonuc = await analyzeContent(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('dokumantasyon_uret', 'Kod için dokümantasyon üret', generateDocsSchema.shape, async (args) => {
        const sonuc = await generateDocs(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('dosya_karsilastir', 'İki dosya içeriğini karşılaştır', compareFilesSchema.shape, async (args) => {
