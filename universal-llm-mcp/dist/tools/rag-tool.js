 ...
 * Bilgi ekle
 */
export async function bilgiEkle(args) {
    const ragService = getRAGService();
    const result = await ragService.addDocument(args.metin, args.kaynak, args.kategori);
    if (result.success) {
 ...
 * Bilgi sorgula (RAG)
 */
export async function bilgiSorgula(args) {
    const ragService = getRAGService();
 ...
 * Bilgileri listele
 */
export async function bilgiListele(args) {
    const ragService = getRAGService();
 ...
 * Kaynak sil
 */
export async function bilgiSil(args) {
    const ragService = getRAGService();
...
export function registerRAGTools(server) {
    server.tool('bilgi_ekle', 'Bilgi tabanına yeni bilgi/belge ekle (RAG için)', bilgiEkleSchema.shape, async (args) => {
        const sonuc = await bilgiEkle(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('bilgi_sorgula', 'Bilgi tabanından akıllı arama yap (RAG ile)', bilgiSorgulaSchema.shape, async (args) => {
        const sonuc = await bilgiSorgula(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('bilgi_listele', 'Bilgi tabanındaki tüm kaynakları listele', bilgiListeleSchema.shape, async (args) => {
        const sonuc = await bilgiListele(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('bilgi_sil', 'Bilgi tabanından kaynak sil', bilgiSilSchema.shape, async (args) => {
