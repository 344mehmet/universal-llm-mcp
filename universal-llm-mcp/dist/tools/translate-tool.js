...
`;
    const response = await router.complete('translate', prompt, `Sen profesyonel bir çevirmensin. ${hedefDil} dilinde mükemmel çeviriler yap.`);
...
`;
    const response = await router.complete('translate', prompt, `Sen kültürlerarası iletişim uzmanısın. Metinleri farklı kültürlere uyarlama konusunda uzmansın.`);
    return response.content;
}
/**
 * Dil algıla
...
belirt.
`;
    const response = await router.complete('translate', prompt, 'Sen dil bilimi uzmanısın. Dilleri ve lehçeleri analiz edebilirsin.');
    return response.content;
}
/**
...
export function registerTranslateTools(server) {
    server.tool('cevir', 'Metni bir dilden başka bir dile çevir', translateSchema.shape, async (args) => {
        const sonuc = await translate(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('yerelleştir', 'Metni hedef kültüre uygun şekilde yerelleştir', localizeSchema.shape, async (args) => {
        const sonuc = await localize(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('dil_algila', 'Metnin dilini algıla ve analiz et', detectLanguageSchema.shape, async (args) => {
        const sonuc = await detectLanguage(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
}
//# sourceMappingURL=translate-tool.js.map