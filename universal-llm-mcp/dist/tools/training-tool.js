 ...
 */
export async function egitimBaslat(args) {
 ...
 * Soru sor
 */
export async function soruSor(args) {
    const session = getTrainingSession();
    if (!session.hasActiveSession()) {
        // Otomatik oturum başlat
        session.start({ category: args.kategori });
    }
 ...
 */
export async function soruSorVeDegerlendir(args) {
    const session = getTrainingSession();
    if (!session.hasActiveSession()) {
        session.start({ category: args.kategori });
    }
    ...
    const llmResponse = await router.complete('default', prompt, 'Soruyu dikkatlice oku ve en iyi cevabını ver. Kısa ve öz ol.');
    // Değerlendir
 ...
 */
export async function cevapDegerlendir(args) {
    const session = getTrainingSession();
 ...
 * İlerleme göster
 */
export async function ilerleseGoster() {
    const session = getTrainingSession();
    if (!session.hasActiveSession()) {
 ...
 */
export async function promptEkle(args) {
    const promptBank = getPromptBank();
 ...
 */
export function registerTrainingTools(server) {
    server.tool('egitim_baslat', 'Yeni bir LLM eğitim oturumu başlat', egitimBaslatSchema.shape, async (args) => {
        const sonuc = await egitimBaslat(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('soru_sor', 'Eğitim için bir test sorusu al', soruSorSchema.shape, async (args) => {
        const sonuc = await soruSor(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('soru_degerlendir', 'Soruyu LLM\'e sor ve cevabını otomatik değerlendir', soruSorSchema.shape, async (args) => {
        const sonuc = await soruSorVeDegerlendir(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('cevap_degerlendir', 'Verilen cevabı değerlendir ve puanla', cevapDegerlendirSchema.shape, async (args) => {
        const sonuc = await cevapDegerlendir(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('ilerleme_goster', 'Eğitim oturumu ilerlemesini ve raporunu göster', {}, async () => {
        const sonuc = await ilerleseGoster();
        return { content: [{ type: 'text', text: sonuc }] };
    });
    server.tool('prompt_ekle', 'Soru bankasına yeni test sorusu ekle', promptEkleSchema.shape, async (args) => {
        const sonuc = await promptEkle(args);
        return { content: [{ type: 'text', text: sonuc }] };
    });
}
//# sourceMappingURL=training-tool.js.map