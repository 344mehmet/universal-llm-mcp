 ...
 * Türkçe sohbet
 */
export async function turkishChat(args: z.infer<typeof turkishChatSchema>): Promise<string> {
 ...
 * Metin özetle
 */
export async function summarize(args: z.infer<typeof summarizeSchema>): Promise<string> {
    const router = getRouter();
...
`;
    ...
    const response = await router.complete('chat', prompt,
        'Sen profesyonel bir özetleme uzmanısın. Metinleri anlaşılır ve özlü şekilde özetle.'
    );
 ...
 * Beyin fırtınası
 */
export async function brainstorm(args: z.infer<typeof brainstormSchema>): Promise<string> {
    const router = getRouter();
...
`;
    ...
    const response = await router.complete('chat', prompt,
        'Sen yaratıcı bir danışmansın. Yenilikçi ve uygulanabilir fikirler üret.'
    );
 ...
 */
export function registerChatTools(server: any): void {
    server.tool(
        'turkce_sohbet',
        'Türkçe sohbet et, sorular sor, yardım al',
        turkishChatSchema.shape,
        async (args: z.infer<typeof turkishChatSchema>) => {
    ...
    server.tool(
        'ozetle',
        'Verilen metni Türkçe olarak özetle',
    ...
    server.tool(
        'beyin_firtinasi',
        'Bir konu hakkında yaratıcı fikirler üret',
        brainstormSchema.shape,
        async (args: z.infer<typeof brainstormSchema>) => {
