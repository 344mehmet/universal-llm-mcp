...
export interface TrainingExample {
    ...
    weight: number; // Regularization ağırlığı
}
...
export class FastTrainer extends EventEmitter {
    ...
    private scoreCache = new FastCache<number>(300); // 5 dk cache
    private isTraining = false;
    private knowledgePool = getSharedKnowledgePool();

    // REG parametreleri
    private regularizationStrength = 0.1;
     ...
     * Eğitim örneği ekle
     */
    public addExample(example: Omit<TrainingExample, 'id' | 'weight'>): string {
     ...
     */
    private calculateWeight(difficulty: number, category: string): number {
     ...
     * Hızlı batch eğitimi
     */
    public async trainBatch(options: Partial<FineTuneBatch> = {}): Promise<TrainingMetrics> {
        if (this.isTraining) {
        ...
        for (const result of allResults) {
            ...
            if (example) {
                ...
                if (result.score > 0.8) {
                    await this.knowledgePool.learnFromTraining(
                        example.input,
                        result.actualOutput,
                        example.category,
                        result.score
     ...
     * Tek örnek eğitimi
     */
    private async trainSingle(example: TrainingExample): Promise<TrainingResult> {
        const startTime = Date.now();
        ...
        try {
            ...
            const response = await withTimeout(
                router.complete('chat', example.input),
                30000, // 30 saniye max
                'Eğitim timeout'
            );
     ...
     */
    private calculateScore(actual: string, expected: string): number {
     ...
     */
    private shuffleWithMomentum(examples: TrainingExample[], epoch: number): TrainingExample[] {
     ...
     * Metrik hesaplama
     */
    private calculateMetrics(results: TrainingResult[]): TrainingMetrics {
     ...
     */
    public async quickTrain(input: string, expected: string, category: string = 'genel'): Promise<TrainingResult> {
     ...
     */
    public setRegularization(strength: number, dropout: number, momentum: number): void {
     ...
     * İstatistikler
     */
    public getStats(): { exampleCount: number; categoryCount: number; isTraining: boolean } {
        return {
...
// Singleton
let trainerInstance: FastTrainer | null = null;

export function getFastTrainer(): FastTrainer {
    if (!trainerInstance) {
