...
export class OllamaBackend extends BaseLLMBackend {
    constructor(url: string, defaultModel: string = 'llama3', timeout: number = 120000) {
    ...
    async listModels(): Promise<string[]> {
        try {
            const response = await this.httpRequest<OllamaModelsResponse>('/api/tags');
    ...
    async complete(request: CompletionRequest): Promise<CompletionResponse> {
        ...
        const ollamaMessages = request.messages.map((msg: ChatMessage) => ({
            role: msg.role,
            content: msg.content,
        }));
        ...
        try {
            const response = await this.httpRequest<OllamaChatResponse>(
                '/api/chat',
                'POST',
                body
            );
